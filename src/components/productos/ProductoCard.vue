        ...
        <div class="flex flex-1 flex-col p-4">
            ...
            <div class="flex flex-1 flex-col justify-end mt-4">
                <p class="text-lg font-bold text-gray-900">{{
                    producto.precios.individual.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' })
                    }}</p>
            </div>
        </div>
        <div class="p-4 pt-0">
...
const sendWhatsAppMessage = () => {
    if (!companyNumber) {
        console.error('⚠️ No se encontró el número de WhatsApp')
        alert('Error: No se encontró el número de WhatsApp de la empresa.')
        return
    }
